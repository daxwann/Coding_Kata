...
def erase_island(grid, coord)
  ...
  grid[y][x] = 0
  erase_island(grid, [y - 1, x]) unless y == 0
  erase_island(grid, [y + 1, x]) unless y == grid.count - 1
  erase_island(grid, [y, x - 1]) unless x == 0
  erase_island(grid, [y, x + 1]) unless x == grid[0].count - 1
end
# rspec
